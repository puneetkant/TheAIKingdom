...
def definition():
    print("=== Eigenvalue Definition: Av = lambdav ===")
    A  = np.array([[4, 1], [2, 3]], dtype=float)
    vals, vecs = np.linalg.eig(A)
    print(f"  A:\n{A}")
    print(f"  eigenvalues  lambda = {vals}")
    print(f"  eigenvectors (columns):\n{np.round(vecs, 4)}")

    # Verify Av = lambdav for each pair
    for i, (lam, v) in enumerate(zip(vals, vecs.T)):
        Av   = A @ v
        lv   = lam * v
        match = np.allclose(Av, lv)
        print(f"  Pair {i}: lambda={lam:.4f}  v={np.round(v,4)}  Av~=lambdav: {match}")
...
def symmetric_matrix():
    ...
    print(f"  A = Q Lambda Q^T? {np.allclose(vecs @ np.diag(vals) @ vecs.T, A)}")
...
def diagonalisation():
    ...
    print(f"  A:\n{A}")
    print(f"  D (diagonal):\n{D}")
    print(f"  P @ D @ P^-1 = A? {np.allclose(P @ D @ Pinv, A)}")
...
def characteristic_poly():
    print("\n=== Characteristic Polynomial det(A - lambdaI) = 0 ===")
    A = np.array([[2, 1], [1, 2]], dtype=float)
    # Expand manually for 2x2: lambda² - tr(A)lambda + det(A)
    tr  = np.trace(A)
    det = np.linalg.det(A)
    print(f"  A:\n{A}")
    print(f"  tr(A)={tr}, det(A)={det}")
    print(f"  char poly: lambda² - {tr}lambda + {det:.0f} = 0")
    roots = np.roots([1, -tr, det])
    print(f"  roots lambda = {np.sort(roots)}")
    print(f"  numpy eig lambda = {np.sort(np.linalg.eig(A)[0])}")
...
def power_iteration_demo():
    ...
    print(f"  Power iteration  lambda_max ~= {lam_power:.6f}")
    print(f"  numpy eig        lambda_max = {np.abs(true_vals[dominant_idx]):.6f}")
    # Align sign
