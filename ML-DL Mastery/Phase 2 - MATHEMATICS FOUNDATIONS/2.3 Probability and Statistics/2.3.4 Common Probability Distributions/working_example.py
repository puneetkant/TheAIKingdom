...
def discrete_distributions():
    ...
    for ax, (rv, label, color) in zip(axes, [
        ...
        (stats.poisson(mu=4),      "Poisson(λ=4)",      "seagreen"),
    ]):
        xs = np.arange(0, 15)
...
def continuous_distributions():
    ...
    dists = {
        "Normal(μ=0,σ=1)":      stats.norm(0, 1),
        "Normal(μ=2,σ=1.5)":    stats.norm(2, 1.5),
        "Exponential(λ=2)":      stats.expon(scale=1/2),
        "Uniform(0,1)":          stats.uniform(0, 1),
        "Gamma(α=3,β=2)":        stats.gamma(a=3, scale=2),
        "Beta(α=2,β=5)":         stats.beta(a=2, b=5),
        "LogNormal(μ=0,σ=0.5)":  stats.lognorm(s=0.5),
    ...
    plot_specs = [
        ...
        (stats.expon(scale=0.5),   "Exponential(λ=2)",    np.linspace(0,5,300)),
        (stats.gamma(a=3,scale=2), "Gamma(α=3,β=2)",      np.linspace(0,20,300)),
        (stats.beta(a=2,b=5),      "Beta(α=2,β=5)",       np.linspace(0,1,300)),
...
def distribution_properties():
    ...
    rv_e = stats.expon(scale=2)   # rate λ=0.5
    s, t = 3, 2
    ...
    lam = n * p_binom
    print(f"\n  Poisson approximation: Bin(n={n},p={p_binom}) ≈ Poisson(λ={lam})")
    for k in range(6):
...
def mle_estimation():
    ...
    print(f"  Normal MLE: μ_true={mu_true}  μ̂={mu_hat:.4f}  σ_true={sigma_true}  σ̂_MLE={sig_hat:.4f}")

    # Exponential data
    lam_true = 3.0
    data_exp = rng.exponential(1/lam_true, 500)
    lam_hat  = 1 / data_exp.mean()   # MLE for exponential
    print(f"  Exponential MLE: λ_true={lam_true}  λ̂={lam_hat:.4f}")

    # Scipy fit
    mu_fit, sig_fit = stats.norm.fit(data)
    print(f"  scipy fit Normal: μ={mu_fit:.4f}  σ={sig_fit:.4f}")
...
def cdf_and_quantiles():
    ...
    print("\n  Quantiles (ppf = percent-point function = Φ⁻¹):")
    for p in [0.025, 0.05, 0.10, 0.50, 0.90, 0.95, 0.975, 0.995]:
        print(f"    Φ⁻¹({p:.3f}) = {rv.ppf(p):.4f}")
