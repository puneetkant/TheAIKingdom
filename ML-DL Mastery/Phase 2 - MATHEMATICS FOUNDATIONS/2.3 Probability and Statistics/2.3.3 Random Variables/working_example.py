...
def continuous_rv():
    ...
    print(f"  PDF f(x) = 1/(b-a) = {1/(b-a):.4f}  for x in [{a},{b}]")
    ...
    print(f"\n  X ~ N({mu},{sigma}²)")
    print(f"  P(mu-sigma <= X <= mu+sigma) = {rv_n.cdf(mu+sigma)-rv_n.cdf(mu-sigma):.4f}  (~=68%)")
    print(f"  P(mu-2sigma <= X <= mu+2sigma)= {rv_n.cdf(mu+2*sigma)-rv_n.cdf(mu-2*sigma):.4f}  (~=95%)")
...
def moments():
    ...
    print(f"  {'Central 3rd  skewnessxsigma³':<30} {moment(3, mu):<12.4f} {0:.4f}  (symmetric)")
    print(f"  {'Central 4th  kurtosis term':<30} {moment(4, mu):<12.4f} {3*sigma**4:.4f}  (3sigma⁴)")
...
def rv_transformations():
    ...
    # Z ~ N(0,1) -> X = sigmaZ + mu ~ N(mu,sigma²)
    Z  = rng.standard_normal(200_000)
    mu, sigma = 10, 3
    X  = sigma * Z + mu
    print(f"  Z~N(0,1) -> X=sigmaZ+mu:  E[X]={X.mean():.4f} (true {mu})  Std[X]={X.std():.4f} (true {sigma})")

    # U ~ Uniform(0,1) -> inverse CDF -> exponential
    U  = rng.uniform(0, 1, 200_000)
    lam = 2.0
    E  = -np.log(1 - U) / lam   # inverse CDF of Exp(lambda)
    print(f"  U~Uniform -> Exp(lambda={lam}): E[X]={E.mean():.4f} (true {1/lam})")

    # Log-normal: Y = exp(X), X ~ N(mu,sigma²)
    mu_n, sigma_n = 0, 1
