...
def maximum_likelihood():
    ...
    # Example 1: Poisson(λ) — MLE is sample mean
    lam_true = 4.5
    data_p   = rng.poisson(lam_true, 200)
    lam_mle  = data_p.mean()   # analytic MLE
    print(f"  Poisson: λ_true={lam_true}  λ_MLE={lam_mle:.4f}  (sample mean)")
    ...
    print(f"\n  Normal: μ_true={mu_true}  μ_MLE={mu_mle:.4f}")
    print(f"          σ_true={sig_true}  σ_MLE={sig_mle:.4f}  σ_unbiased={sig_unb:.4f}")

    # Example 3: Numeric MLE via log-likelihood maximisation
    # X ~ Weibull(shape=k, scale=λ) — no closed form
    ...
    print(f"\n  Weibull numeric MLE: k_true={k_true}  k̂={k_hat:.4f}  λ_true={lam_w}  λ̂={lam_hat:.4f}")
...
def bias_variance_mse():
    ...
    print(f"  True μ={mu}, σ={sigma}, n={n}, M={M} simulations")
    for name, est, true_val in [
        ("X̄ (mean)",       mu_hats,   mu),
        ("σ_MLE (ddof=0)",  sigma_mle, sigma),
        ("σ_unb (ddof=1)",  sigma_unb, sigma),
    ]:
        bias = est.mean() - true_val
        var  = est.var()
        mse  = np.mean((est - true_val)**2)
        print(f"\n  {name:<22}: E[θ̂]={est.mean():.4f}  bias={bias:.4f}  var={var:.4f}  MSE={mse:.4f}")
    print(f"\n  Note: MSE = bias² + variance")
    b = sigma_mle.mean() - sigma
    print(f"  σ_MLE: bias²={b**2:.4f} + var={sigma_mle.var():.4f} = {b**2+sigma_mle.var():.4f} ≈ MSE={np.mean((sigma_mle-sigma)**2):.4f}")


# ── 4. Cramér-Rao lower bound ────────────────────────────────────────────────
def cramer_rao():
    print("\n=== Cramér-Rao Lower Bound Var(θ̂) ≥ 1/I(θ) ===")
    # X ~ Poisson(λ):  Fisher info I(λ) = n/λ  → CRLB = λ/n
    lam, n = 3.0, 50
    crlb   = lam / n
    print(f"  Poisson(λ={lam}), n={n}:")
    print(f"  Fisher info I(λ) = n/λ = {n/lam:.4f}")
    print(f"  CRLB = 1/I(λ) = λ/n = {crlb:.4f}")
    print(f"  Var(λ̂_MLE) = λ/n = {crlb:.4f}  (MLE is efficient, achieves bound)")

    rng      = np.random.default_rng(5)
    M        = 50_000
    lam_hats = np.array([rng.poisson(lam, n).mean() for _ in range(M)])
    print(f"  Simulated Var(λ̂) = {lam_hats.var():.6f}  (CRLB = {crlb:.6f})")
...
def confidence_intervals():
    ...
    # 95% CI for mean (unknown σ — use t-distribution)
    ...
    print(f"  Data: n={n}  x̄={data.mean():.4f}  s={data.std(ddof=1):.4f}")
    print(f"  95% CI (known σ):   ({ci_z[0]:.4f}, {ci_z[1]:.4f})")
    print(f"  95% CI (unknown σ): ({ci_t[0]:.4f}, {ci_t[1]:.4f})")
    print(f"  scipy:              ({ci_scipy[0]:.4f}, {ci_scipy[1]:.4f})")
    print(f"  True μ={mu}  inside known σ CI? {ci_z[0] <= mu <= ci_z[1]}")
