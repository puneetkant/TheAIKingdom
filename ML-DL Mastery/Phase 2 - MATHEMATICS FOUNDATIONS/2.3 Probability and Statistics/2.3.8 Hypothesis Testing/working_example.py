...
def framework():
    ...
    print("  H0: null hypothesis (claim to be tested)")
    print("  H1: alternative hypothesis")
    print("  alpha  = P(Type I error)  = P(reject H0 | H0 true)  — significance level")
    print("  beta  = P(Type II error) = P(fail to reject H0 | H1 true)")
    print("  Power = 1 - beta = P(reject H0 | H1 true)")
    print("  p-value = P(observing test stat >= observed | H0) — reject if p < alpha")
...
def z_test():
    ...
    print(f"  H0: mu={mu0}  True mu={mu1}  sigma={sigma}  n={n}")
    print(f"  x={data.mean():.4f}  z={z_stat:.4f}  z_crit=+/-{z_crit:.4f}")
...
def t_tests():
    ...
    print(f"  One-sample t-test (H0: mu=5):")
    print(f"    t={t:.4f}  p={p:.4f}  {'reject' if p<0.05 else 'fail'} H0 at alpha=0.05")
    ...
    print(f"\n  Two-sample t-test (H0: mu_A=mu_B):")
    print(f"    x_A={A.mean():.4f}  x_B={B.mean():.4f}")
    print(f"    t={t2:.4f}  p={p2:.4f}  {'reject' if p2<0.05 else 'fail'} H0")
    ...
    print(f"\n  Welch's t-test (H0: mu_C=mu_D, unequal var):")
    print(f"    t={t3:.4f}  p={p3:.4f}  {'reject' if p3<0.05 else 'fail'} H0")

    # Paired t-test
    before = rng.normal(70, 5, 20)
    after  = before - rng.normal(3, 2, 20)   # treatment effect ~= 3
    t4, p4 = stats.ttest_rel(before, after)
    print(f"\n  Paired t-test (before/after, H0: Delta=0):")
    print(f"    mean diff={np.mean(before-after):.4f}  t={t4:.4f}  p={p4:.4f}  {'reject' if p4<0.05 else 'fail'} H0")
...
def chi_squared_tests():
    ...
    print(f"    chi²={chi2:.4f}  p={p:.4f}  {'reject' if p<0.05 else 'fail'} H0 at alpha=0.05")
    ...
    print(f"    chi²={chi2_ind:.4f}  p={p_ind:.4f}  df={dof}  {'reject' if p_ind<0.05 else 'fail'} independence")
...
# -- 5. ANOVA ------------------------------------------------------------------
def anova():
    print("\n=== One-Way ANOVA (H0: all group means equal) ===")
    ...
    print(f"  F={F:.4f}  p={p:.4f}  {'reject H0 (>=1 means differ)' if p<0.05 else 'fail to reject H0'}")
    ...
    print(f"\n  Post-hoc (Bonferroni alpha_adj={alpha_bonf:.4f}):")
    for a, b in pairs:
...
def power_analysis():
    ...
    print(f"  Effect size d={d:.2f}  alpha={alpha}  beta={beta}  power={1-beta:.0%}")
    print(f"  Required n per group ~= {int(np.ceil(n_req))}")

    # Power curve vs n
    print(f"\n  Power at various n (two-sample z-test):")
    print(f"  {'n':<6} {'power':<10} {'beta'}")
...
def multiple_testing():
    ...
    print(f"  100 tests (10 signal, 90 null)  alpha={alpha}")
    print(f"  {'Method':<20} {'Rejections'}")
    print(f"  {'Uncorrected':<20} {reject_raw}")
    print(f"  {'Bonferroni':<20} {reject_bonf}  (alpha_adj={alpha/m:.4f})")
