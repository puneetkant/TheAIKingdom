...
def sample_spaces():
    ...
    print(f"  A∩B: {A&B}")
    print(f"  A∪B: |A∪B|={len(A|B)}  P(A∪B)={Fraction(len(A|B), n)}")
...
def kolmogorov_axioms():
    ...
    print(f"  A∩B={A&B}  (disjoint)   P(A∪B)={pAuB}  = P(A)+P(B)={pA+pB}  ✓")
...
def conditional_probability():
    ...
    print(f"  P(Ace ∩ Spade)   = {pAB:.4f} = {Fraction(1,52)}")
...
def total_probability():
    ...
    # P(B) = Σ P(B|Ai)·P(Ai)  where {Ai} partition Ω
...
def monte_carlo():
    ...
    # Estimate π via circle inscribed in unit square
    xs, ys = rng.uniform(-1, 1, N), rng.uniform(-1, 1, N)
    inside = (xs**2 + ys**2 <= 1).sum()
    pi_est = 4 * inside / N
    print(f"  π estimate (N={N:,}): {pi_est:.5f}  (true: {np.pi:.5f})")
