...
def dqn_theory():
    ...
    print()
    print("  Core idea: approximate Q(s,a) with neural network θ")
    print("  Loss: L(θ) = E[(r + γ max_{a'} Q(s',a';θ⁻) - Q(s,a;θ))²]")
    print()
    ...
    print()
    print("  2. Target Network θ⁻:")
    print("     Separate network for computing targets")
    print("     Update θ⁻ ← θ every C steps → stable targets")
    print()
    print("  DQN variants:")
    variants = [
...
def dqn_demo():
    ...
    n_actions = 2; state_dim = 4; γ = 0.99; lr = 0.005
...
def policy_gradient():
    ...
    for ep in range(300):
        ...
        dW1 = np.zeros_like(π_net.W1); db1 = np.zeros_like(π_net.b1)
        dW2 = np.zeros_like(π_net.W2); db2 = np.zeros_like(π_net.b2)
        for (si, ai, pi), adv in zip(h_cache, advs):
            h1   = relu(si @ π_net.W1 + π_net.b1)
            dlog = -pi.copy(); dlog[ai] += 1   # ∇ log π
            g    = -adv * dlog  # gradient direction
            dW2 += np.outer(h1, g)
            db2 += g
            dh1  = (g @ π_net.W2.T) * (h1 > 0)
            dW1 += np.outer(si, dh1)
            db1 += dh1

        n = len(h_cache)
        π_net.apply_grads([dW1/n, db1/n, dW2/n, db2/n], lr)
...
def actor_critic():
    ...
    print("  Actor: policy π_θ(a|s)  ← optimised by policy gradient")
    print("  Critic: value V_φ(s)    ← bootstraps advantage estimates")
    print()
    print("  A2C update:")
    print("    δ_t = R_t + γV(s_{t+1}) - V(s_t)   (TD error = advantage estimate)")
    print("    θ ← θ + α · δ_t · ∇ log π_θ(a_t|s_t)   (actor)")
    print("    φ ← φ + β · δ_t · ∇V_φ(s_t)              (critic)")
    print()
...
def ppo_overview():
    ...
    print("    r_t(θ) = π_θ(a_t|s_t) / π_{θ_old}(a_t|s_t)")
    print("    L^CLIP = E[min(r_t · A_t, clip(r_t, 1-ε, 1+ε) · A_t)]")
    print("    ε typically 0.1-0.2  (how far new policy can stray)")
    print()
    print("  Full loss:")
    print("    L = L^CLIP - c₁·L_VF + c₂·S[π_θ]")
    print("    L_VF = (V_θ(s_t) - V_t^target)²   (value loss)")
    print("    S[π]  = -Σ π log π                 (entropy bonus)")
    print()
    print("  Training loop:")
    steps = [
        "1. Collect T timesteps of experience with π_{θ_old}",
        "2. Compute advantages (GAE-λ: generalised advantage estimation)",
    ...
    for s in steps:
        print(f"  {s}")
    print()
    print("  GAE: Â_t = Σ_{l=0}^∞ (γλ)^l δ_{t+l}  (smoothed advantage)")
    print()
