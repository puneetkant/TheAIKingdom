...
def mpnn_framework():
    ...
    print("    m_v^{k+1} = AGG_{u ∈ N(v)} MSG(h_v^k, h_u^k, e_{uv})")
...
def gcn_demo():
    ...
    print("  H^{l+1} = σ(Ã H^l W^l)")
    print("  Ã = D̃^{-1/2}(A+I)D̃^{-1/2}  (normalised adjacency with self-loops)")
    print()
...
def graphsage_demo():
    ...
    print("  h_v^l = σ(W · CONCAT(h_v^{l-1}, AGG_{u ∈ N_S(v)} h_u^{l-1}))")
    print()
...
def gat_demo():
    ...
    print("    α_{vw} = softmax_w(e_{vw})")
    print("    h_v' = σ(Σ_{w ∈ N(v)} α_{vw} · W h_w)")
    print()
...
def gin_demo():
    ...
    print("  h_v^{k} = MLP^k ((1+ε)·h_v^{k-1} + Σ_{u∈N(v)} h_u^{k-1})")
    print()
